...
class GetHourKline():
    ...
    # 定义一个函数，传入期权合约代码和日期，通过币安期权API获取小时k线数据
    def get_kline(self, symbol, start_hour, end_hour, length=100):
        ...
        while True:
            try:
                ...
                # 检查API是否返回错误信息
                if isinstance(klines_raw, dict) and 'code' in klines_raw:
                    print(f"API错误 - Symbol: {symbol}, 错误信息: {klines_raw}")
                    klines_data = []
                    break
                
                # 正常返回但数据为空：不重试，直接退出循环
                if not klines_raw:
                    # print(f"DEBUG: API返回空数据 - Symbol: {symbol}, 时间范围: {start_hour} to {end_hour}")
                ...
                for item in klines_raw:
                    try:
                        ...
                        print(f"警告: {symbol} 数据格式异常，缺少字段: {e}, 数据项: {item}")
                        continue
                break
            except Exception as e:
    ...
    def get_all_kline(self):
        ...
        if is_null:
            ...
            date_str_of_last_file = date_start
            last_feather_path = data_path['result_path_hour'] / rf'{date_str_of_last_file}.feather'
            df = pd.read_feather(last_feather_path)
            df["开盘时间"] = df["开盘时间"].astype(str)
            df["开盘时间"] = pd.to_datetime(df["开盘时间"])
            hour_start = (df["开盘时间"].max() + timedelta(hours=1)).tz_localize(None)
        ...
        # 调试信息：显示时间范围
        print(f"时间范围: {hour_start} 到 {hour_end}, 共 {(hour_end - hour_start).total_seconds() / 3600:.1f} 小时")
        print(f"Symbols数量: {len(symbols)} (将处理所有symbols)")
        if len(symbols) > 0:
            print(f"示例symbols: {symbols[:3] if len(symbols) >= 3 else symbols}")

        hour_list = pd.date_range(hour_start, hour_end, freq='1h')
        batch_size = 1488  # 62天（24小时 × 62天），24的倍数
        hour_list = [hour_list[i:i + batch_size] for i in range(0, len(hour_list), batch_size)]
        
        # 调试信息：显示批次信息
        print(f"时间范围被分成 {len(hour_list)} 个批次（每批{batch_size}小时，即{batch_size//24}天）")

        # 处理所有批次，并在每个批次结束后立即保存
        for hours in hour_list:
            print(f"开始处理批次：{hours[0]} 到 {hours[-1]}，共 {len(hours)} 小时")
            batch_desc = hours[0].strftime('%Y-%m-%d %H:%M') + " to " + hours[-1].strftime('%Y-%m-%d %H:%M')
            for symbol in tqdm(symbols, desc=batch_desc):
                ...
                if not kline_df.empty:
                    ...
                    # 调试信息：显示为什么没有数据
                    print(f"\n警告: {symbol} 在时间范围 {hours[0]} 到 {hours[-1]} 没有数据")
            ...
            # 统一开盘时间格式为字符串（如果是从feather读取的，可能是datetime类型）
            df['开盘时间'] = df['开盘时间'].astype(str)
            ...
            for sub_date, sub_df in dfgroup:
                # 确保开盘时间是datetime类型，并且按时间排序
                del sub_df['日期']
                sub_df['开盘时间'] = sub_df['开盘时间'].astype(str)
                sub_df = sub_df.sort_values(['开盘时间', '货币对']).reset_index(drop=True)

                feather_path = data_path['result_path_hour'] / rf'{sub_date}.feather'

                sub_df.reset_index(inplace=True, drop=True)
                # 默认保存为feather文件
                sub_df.to_feather(feather_path)
                print(f'{sub_date} 小时K线数据已保存为feather文件（批次 {batch_desc}）')
                
                # 如果设置了save_csv，同时保存CSV文件
                if self.save_csv:
                    csv_path = data_path['result_path_hour'] / rf'{sub_date}.csv'
                    sub_df.to_csv(csv_path, index=False, encoding='utf-8-sig')
                    print(f'{sub_date} 小时K线数据已保存为CSV文件（批次 {batch_desc}）')
