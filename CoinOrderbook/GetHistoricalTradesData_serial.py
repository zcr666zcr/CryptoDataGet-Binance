...
class GetHistoricalTradesData():
    ...
    def get_historical_trades_by_fromid(self, from_id=None, limit=None):
        ...
        while request_count < max_requests:
            try:
                ...
                # 获取当前批次的最小ID和最大ID
                min_id_in_batch = min(trade['id'] for trade in trades_data)
                max_id_in_batch = max(trade['id'] for trade in trades_data)
                last_id = trades_data[-1]['id']
                
                # 防止无限循环：如果当前fromId大于批次中的最小ID，说明已经到达最新数据
                # 这种情况通常发生在已经获取到最新数据后，API可能返回重复数据或空数据
                if current_from_id is not None and current_from_id > min_id_in_batch:
                    print(f"检测到已到达最新数据：当前fromId({current_from_id}) > 批次最小ID({min_id_in_batch})，停止获取")
                    # 只添加当前批次中ID大于等于current_from_id的数据（避免重复）
                    new_trades = [trade for trade in trades_data if trade['id'] >= current_from_id]
                    if new_trades:
                ...
                if len(trades_data) < limit:
                    print(f"返回数据少于limit({len(trades_data)} < {limit})，已获取完所有数据")
                    break
                
                # 检查ID是否还在正常增长（防止无限循环）
                # 如果当前批次的最大ID小于等于当前fromId，说明可能已经到达最新数据
                # 正常情况下，max_id_in_batch 应该大于 current_from_id
                if current_from_id is not None and max_id_in_batch <= current_from_id:
                    print(f"检测到ID增长异常（max_id={max_id_in_batch} <= fromId={current_from_id}），可能已到达最新数据，停止获取")
                    break
                ...
                print(f"已获取 {len(all_data)} 条记录，继续获取... (当前批次ID范围: {min_id_in_batch} - {max_id_in_batch})")
        ...
        if duplicate_count > 0:
            print(f"检测到并删除了 {duplicate_count} 条重复记录（基于成交ID）")
    ...
    def save_historical_trades_by_date(self, df, save_format=None, delete_old_files=True):
        """
        保存历史成交数据到文件（按日期合并模式）
        如果新数据在同一天，就与文件夹中同样日期的文件合并
        ...
        # 按日期合并新数据
        data_dir = data_path['result_path_historical_trades']
        file_paths = save_and_merge_by_date(
            data_dir=data_dir,
            symbol=self.symbol,
            new_df=df,
            save_format=save_format,
        ...
        if file_paths:
            print(f'历史成交数据已保存（按日期合并模式）：共 {len(file_paths)} 个文件')
    ...
    def update_from_local(self, limit=None, save_format=None, max_requests=None, merge_by_date=True, delete_old_files=True):
        """
        ...
        3. 保存新数据：
           - 如果 merge_by_date=True：按日期合并（如果新数据在同一天，就与文件夹中同样日期的文件合并）
           - 如果 merge_by_date=False：只保存新数据，不合并不删除旧文件（合并操作在读取时进行）
        
        :param limit: 每次请求返回数量，最大1000，默认从配置文件读取
        :param save_format: 保存格式，'feather' 或 'csv'，默认从配置文件读取
        :param max_requests: 最大请求次数，防止无限循环，默认从配置文件读取
        :param merge_by_date: 是否使用按日期合并模式，默认True（推荐用于定时更新）
        :param delete_old_files: 是否删除已合并的旧文件（仅在merge_by_date=True时有效），默认True
        ...
        if merge_by_date:
            # 使用按日期合并模式
            print("使用按日期合并模式：如果新数据在同一天，将与文件夹中同样日期的文件合并")
            file_paths = save_and_merge_by_date(
                data_dir=data_dir,
                symbol=self.symbol,
                new_df=new_df,
            ...
            if file_paths:
                ...
                print(f"共保存 {len(file_paths)} 个文件（按日期合并）")
                return new_df
            else:
                print("保存失败")
                return pd.DataFrame()
    ...
    def run_all_tasks(self, use_split=None, merge_by_date=True, delete_old_files=True):
        """
        执行所有任务（定时更新）
        ...
        try:
            df = self.update_from_local(merge_by_date=merge_by_date, delete_old_files=delete_old_files)
            if not df.empty:
    ...
    def start_scheduler(self, is_scheduler=False, interval_minutes=1, use_split=None, merge_by_date=True, delete_old_files=True):
        """
        启动定时任务
        ...
        if not is_scheduler:
            # 立即执行一次
            self.run_all_tasks(use_split=use_split, merge_by_date=merge_by_date, delete_old_files=delete_old_files)
        else:
